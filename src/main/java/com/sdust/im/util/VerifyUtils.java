...
public class VerifyUtils {
	...
	public static boolean matchAccount(String text) {
		if (Pattern.compile("^[a-zA-Z0-9_-]{6,18}$").matcher(text).matches()) {
			return true;
		}
		return false;
	}

	public static boolean matchAccount1(String text) {
		if (Pattern.compile("^(\\d{17})([0-9]|[xX])$").matcher(text).matches()) {
			return true;
		}
		return false;
	}
