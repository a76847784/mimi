...
public class FriendMessageAdapter extends BaseAdapter {
	...
	public FriendMessageAdapter(Context context, List<MessageTabEntity> vector,List<User> users) {
		this.mMessageEntities = vector;
		this.users = users;
		mInflater = LayoutInflater.from(context);
		mContext0 = context;
...
//		0620
		Bitmap photo = null ;
		for (User user: users) {
			if (user.getId()==senderId.intValue()){
				photo = PhotoUtils.getBitmap(user.getPhoto());
				break;
			}
		}
//		Bitmap photo = PhotoUtils.getBitmap(message.getPhoto());
//		0620
